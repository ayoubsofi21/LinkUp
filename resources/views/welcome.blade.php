<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" x-data="{ darkMode: localStorage.getItem('darkMode') === 'true', mobileMenu: false, faqOpen: null }" x-init="$watch('darkMode', val => localStorage.setItem('darkMode', val))" :class="{ 'dark': darkMode }">
<head>
    <meta charset="utf-8">
    ...
    <div class="container-lg">
        <nav class="flex items-center gap-6 h-16">
            ...
            <div class="flex items-center gap-3 ml-auto">
                ...
                @auth
                    ...
                    <a href="{{ route('feed') }}" class="btn-primary text-sm px-4 py-2">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" class="flex items-center gap-2 pl-1 pr-3 py-1 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                            ...
                            <span class="text-sm font-medium text-slate-700 dark:text-slate-300 hidden md:block">{{ Auth::user()->name }}</span>
        ...
        <div x-show="mobileMenu" x-transition class="lg:hidden border-t border-slate-200 dark:border-slate-800 py-4 space-y-1">
            ...
            <div class="pt-2 flex gap-3">
                @guest
                    <a href="{{ route('login') }}" class="btn-secondary flex-1 text-center text-sm justify-center">Sign in</a>
                    <a href="{{ route('register') }}" class="btn-primary flex-1 text-center text-sm justify-center">Get started</a>
                @endguest
            </div>
        </div>
    </div>
    ...
    <div class="container-lg relative z-10 grid lg:grid-cols-2 gap-16 items-center">
        ...
        <div class="space-y-8 fade-in-up">
            ...
            <div class="flex flex-wrap gap-4">
                @auth
                    <a href="{{ route('feed') }}" class="btn-primary text-base px-7 py-3.5">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="mockup-body" style="background: #0f172a;">
                    <div class="grid grid-cols-3 gap-3 h-full">
                        ...
                        <div class="col-span-2 space-y-2.5">
                            @foreach([['#2563EB','#4F46E5','w-28','w-36'],['#4F46E5','#8B5CF6','w-20','w-44'],['#0EA5E9','#2563EB','w-32','w-28']] as $post)
                            <div class="rounded-xl p-3" style="background: rgba(30,41,59,0.6); border: 1px solid rgba(255,255,255,0.05);">
                                <div class="flex items-center gap-2 mb-2.5">
                                    <div class="w-7 h-7 rounded-full" style="background: linear-gradient(135deg, {{$post[0]}}, {{$post[1]}});"></div>
                                    <div>
                                        <div class="h-2 rounded {{$post[2]}}" style="background: #334155; margin-bottom: 3px;"></div>
                                        <div class="h-1.5 rounded w-16" style="background: #1e293b;"></div>
                                    </div>
                                </div>
                                <div class="space-y-1.5 mb-2.5">
                                    <div class="h-2 rounded {{$post[3]}}" style="background: #1e293b;"></div>
    ...
    <div class="container-lg">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div class="stat-card text-center glass-card" style="background: linear-gradient(135deg, rgba(255,255,255,0.9), rgba(248,250,252,0.8));">
                <div class="text-4xl font-black mb-1" style="background: linear-gradient(135deg, {{$s[2]}}, {{$s[3]}}); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
                    {{ $s[0] }}
                </div>
                <div class="text-base font-semibold text-slate-800 dark:text-slate-100 mb-2">{{ $s[1] }}</div>
                <p class="text-xs text-slate-500 dark:text-slate-400 leading-snug">{{ $s[2] ?? '' }}</p>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="container-lg">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="glass-card p-7 group">
                <div class="w-12 h-12 rounded-2xl flex items-center justify-center mb-5 {{ $f[0] }} transition-transform group-hover:scale-110">
                    <svg class="w-6 h-6" style="color: {{ $f[1] }};" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
                        {!! $f[4] !!}
                    </svg>
                </div>
                <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">{{ $f[2] }}</h3>
                <p class="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">{{ $f[3] }}</p>
                <div class="mt-4 flex items-center gap-1 text-sm font-semibold transition-all group-hover:gap-2" style="color: {{ $f[1] }};">
                    Learn more
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path d="M5 12h14M12 5l7 7-7 7"/>
                    </svg>
            ...
            <div class="rounded-3xl overflow-hidden" style="background: linear-gradient(135deg, #0f172a, #1e1b4b); min-height: 420px; padding: 32px; position: relative;">
                ...
                <div class="absolute inset-0 flex items-center justify-center opacity-20">
                    <svg viewBox="0 0 400 300" class="w-full h-full">
                        ...
                        @endphp
                        @foreach($edges as $e)
                        <line x1="{{ $nodes[$e[0]][0] }}" y1="{{ $nodes[$e[0]][1] }}" x2="{{ $nodes[$e[1]][0] }}" y2="{{ $nodes[$e[1]][1] }}" stroke="white" stroke-width="1" opacity="0.3"/>
                        @endforeach
                        @foreach($nodes as $i => $n)
                        <circle cx="{{ $n[0] }}" cy="{{ $n[1] }}" r="{{ $i === 0 ? 16 : 8 }}" fill="{{ $i === 0 ? '#2563EB' : 'white' }}" opacity="{{ $i === 0 ? '1' : '0.7' }}"/>
                        @endforeach
                    </svg>
                </div>
                ...
                <div class="relative z-10 space-y-4">
                    ...
                    @foreach([['Alex M.', 'Senior Engineer @ Vercel', '#2563EB','#4F46E5'],['Priya S.', 'Open Source Maintainer', '#4F46E5','#8B5CF6'],['Jorge R.', 'DevRel @ GitHub', '#0EA5E9','#2563EB']] as $dev)
                    <div class="flex items-center gap-3 p-3 rounded-xl" style="background: rgba(255,255,255,0.07); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.1);">
                        <div class="w-10 h-10 rounded-full flex-shrink-0" style="background: linear-gradient(135deg, {{ $dev[2] }}, {{ $dev[3] }});"></div>
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-semibold text-white">{{ $dev[0] }}</div>
                            <div class="text-xs text-slate-400 truncate">{{ $dev[1] }}</div>
                        </div>
        ...
        <div class="space-y-8">
            ...
            <div class="space-y-5">
                ...
                <div class="flex gap-4">
                    ...
                    <div>
                        <div class="font-semibold text-slate-900 dark:text-white mb-0.5">{{ $b[0] }}</div>
                        <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">{{ $b[1] }}</p>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            @guest
            <a href="{{ route('register') }}" class="btn-primary w-fit">
                Join for free
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                    <path d="M5 12h14M12 5l7 7-7 7"/>
                </svg>
                ...
                <div class="relative glass-card p-6 group">
                    ...
                    <div class="absolute -left-16 top-6 w-[58px] flex items-center justify-center">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-white text-xs font-black shadow-lg group-hover:scale-110 transition-transform"
                             style="background: linear-gradient(135deg, {{ $step[3] }}, {{ $step[4] }});">
                            {{ $step[0] }}
                        </div>
                    </div>

                    <h3 class="font-bold text-slate-900 dark:text-white mb-1.5">{{ $step[1] }}</h3>
                    <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">{{ $step[2] }}</p>
                </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-3 gap-6">
            ...
            <div class="glass-card p-7 flex flex-col group">
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <div class="w-10 h-10 rounded-full flex-shrink-0" style="background: linear-gradient(135deg, {{ $t[3] }}, {{ $t[4] }});"></div>
                    <div>
                        <div class="font-semibold text-slate-900 dark:text-white text-sm">{{ $t[0] }}</div>
                        <div class="text-xs text-slate-500 dark:text-slate-400">{{ $t[1] }} · {{ $t[2] }}</div>
                    </div>
                </div>
            </div>
            @endforeach
    ...
    <div class="container-lg">
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            ...
            <div class="glass-card p-6">
                ...
                <div class="space-y-4">
                    @foreach([['Ana Lima','Rust·WebAssembly','#F97316','#EF4444','+842'],['Kai Chen','Go·Kubernetes','#2563EB','#4F46E5','+631'],['Mia Hoffman','TypeScript·React','#8B5CF6','#0EA5E9','+519']] as $dev)
                    <div class="flex items-center gap-3 group cursor-pointer">
                        <div class="w-9 h-9 rounded-full flex-shrink-0 transition-transform group-hover:scale-105" style="background: linear-gradient(135deg, {{ $dev[2] }}, {{ $dev[3] }});"></div>
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-semibold text-slate-900 dark:text-white">{{ $dev[0] }}</div>
                            <div class="text-xs text-slate-500">{{ $dev[1] }}</div>
                        </div>
                        <span class="text-xs font-bold text-emerald-500">{{ $dev[4] }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="glass-card p-6">
                ...
                <div class="space-y-4">
                    @foreach([['devlink-cli','A CLI for managing dev environments','TypeScript','⭐ 2.1k'],['rustwebgl','WebGL bindings in pure Rust','Rust','⭐ 1.8k'],['openreview','AI-powered code review platform','Go','⭐ 964']] as $proj)
                    <div class="group cursor-pointer">
                        <div class="flex items-start justify-between gap-2 mb-1">
                            <div class="text-sm font-semibold text-blue-600 dark:text-blue-400 group-hover:underline">{{ $proj[0] }}</div>
                            <span class="text-xs text-slate-500 flex-shrink-0">{{ $proj[3] }}</span>
                        </div>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mb-1.5">{{ $proj[1] }}</p>
                        <span class="tech-badge">{{ $proj[2] }}</span>
                    </div>
                    @endforeach
                </div>
            </div>

            {{-- Tech Tags + Latest Posts --}}
            <div class="glass-card p-6">
                <h3 class="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                ...
                <div class="flex flex-wrap gap-2 mb-6">
                    @foreach(['#rust','#typescript','#webassembly','#llm','#kubernetes','#react','#go','#devops','#openai','#nextjs'] as $tag)
                    <a href="#" class="tech-badge hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors">{{ $tag }}</a>
                    @endforeach
                </div>
                <h4 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">Latest Posts</h4>
                <div class="space-y-3">
                    @foreach([['Building a type-safe API client in Go','3 min read'],['Why I moved my team from React to Svelte','7 min read'],['Zero-cost abstractions in Rust, explained','5 min read']] as $post)
                    <div class="group cursor-pointer">
                        ...
                        <div class="text-xs text-slate-400 mt-0.5">{{ $post[1] }}</div>
                    </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="space-y-3">
            ...
            <div class="glass-card overflow-hidden"
                ...
                <button @click="open = !open"
                        ...
                        :class="open ? 'pb-3' : ''">
                    <span class="font-semibold text-slate-900 dark:text-white text-sm md:text-base">{{ $faq[0] }}</span>
                ...
                <div x-show="open" x-collapse class="px-5 pb-5">
                    <p class="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">{{ $faq[1] }}</p>
                </div>
            </div>
            @endforeach
        </div>
...
<section class="py-24 relative overflow-hidden cta-gradient">
    ...
    <div class="container-lg relative z-10 text-center space-y-8">
        ...
        <div class="flex flex-wrap justify-center gap-4 pt-2">
            @auth
                <a href="{{ route('feed') }}" class="btn-primary text-base px-8 py-4">
                    Go to your feed
...
<footer class="bg-slate-950 text-slate-400 border-t border-slate-800">
    <div class="container-lg py-16">
        <div class="grid grid-cols-2 md:grid-cols-5 gap-8 mb-12">
            ...
            <div class="col-span-2 md:col-span-1 space-y-4">
                ...
                <div class="flex gap-3">
                    ...
                    <a href="#" title="{{ $s[0] }}" class="w-8 h-8 rounded-lg bg-slate-800 hover:bg-slate-700 flex items-center justify-center transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            {!! $s[1] !!}
                        </svg>
                    </a>
            ...
            <div class="space-y-4">
                ...
                <ul class="space-y-2.5 text-sm">
                    @foreach(['Feed','Profiles','Projects','Messaging','Jobs','Community'] as $item)
            ...
            <div class="space-y-4">
                ...
                <ul class="space-y-2.5 text-sm">
                    @foreach(['About','Blog','Careers','Press','Brand Kit'] as $item)
                    <li><a href="#" class="hover:text-white transition-colors">{{ $item }}</a></li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="space-y-4">
                ...
                <ul class="space-y-2.5 text-sm">
                    @foreach(['Documentation','API','Changelog','Status','Roadmap'] as $item)
                    <li><a href="#" class="hover:text-white transition-colors">{{ $item }}</a></li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="space-y-4">
                ...
                <ul class="space-y-2.5 text-sm">
                    @foreach(['Privacy Policy','Terms of Service','Cookie Policy','Security','GDPR'] as $item)
                    <li><a href="#" class="hover:text-white transition-colors">{{ $item }}</a></li>
                    @endforeach
                </ul>
            </div>
        ...
        <div class="pt-8 border-t border-slate-800 flex flex-col md:flex-row items-center justify-between gap-4">
            <p class="text-sm">© {{ date('Y') }} LinkUp. All rights reserved.</p>
